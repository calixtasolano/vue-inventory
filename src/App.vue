...
export default {
      ...
      westdata: [
        {
          ...
          titleUrl:
            "https://www.google.com/search?q=Harry+Potter+and+the+Sorcerer%27s+Stone",
          author: " J. K. Rowling",
          quantity: "10",
          isbn: "0399501487",
          date: "2022-05-30",
        },
        {
          ...
          titleUrl: "https://www.google.com/search?q=lord+of+the+flies",
        ...
        },
        {
          ...
          titleUrl:
            "https://www.google.com/search?q=roll+of+thunder+hear+my+cry",
  ...
  methods: {
    ...
    addItem: function (arr) {
      ...
      if (!this.date) {
        alert(
          "You must enter a complete date. Your entry will be made without a date."
        );
      }
      if (this.isbn) {
        this.checkISBN();
    ...
    //Validate ISBN taken from O'Reilly: https://www.oreilly.com/library/view/regular-expressions-cookbook/9781449327453/ch04s13.html
    checkISBN: function () {
      var subject = this.isbn;

      // Checks for ISBN-10 or ISBN-13 format
      var regex =
        /^(?:ISBN(?:-1[03])?:? )?(?=[0-9X]{10}$|(?=(?:[0-9]+[- ]){3})[- 0-9X]{13}$|97[89][0-9]{10}$|(?=(?:[0-9]+[- ]){1,4})[- 0-9]{14,17}$)(?:97[89][- ]?)?[0-9]{1,5}[- ]?[0-9]+[- ]?[0-9]+[- ]?[0-9X]$/;
      ...
      if (regex.test(subject)) {
        ...
        if (check == last) {
          ...
          alert(
            "Invalid ISBN check digit. Your entry will be made without the ISBN."
          );
          return (this.isbn = "");
        }
      } else {
